...
// migrateSettingsThemeMatteBlack renames the omarchy slug to matte-black and
// the og slug to rose-pine-dark, replacing the previous rose-pine-dark palette
// with the former og colors. Stored rose-pine-dark values already use the
// surviving slug, so only omarchy and og rows are remapped.
func migrateSettingsThemeMatteBlack(ctx context.Context, tx *sql.Tx) error {
	var tableCount int
	if err := tx.QueryRowContext(ctx, `
		SELECT COUNT(*)
		FROM sqlite_master
	...
	if _, err := tx.ExecContext(ctx, `CREATE TABLE settings_theme_19 (
		id INTEGER PRIMARY KEY CHECK (id = 1),
		default_model TEXT,
		max_tool_rounds INTEGER CHECK (max_tool_rounds IS NULL OR max_tool_rounds BETWEEN 1 AND 100),
		default_tools_configured INTEGER NOT NULL DEFAULT 0 CHECK (default_tools_configured IN (0, 1)),
		...
		ntfy_topic TEXT,
		ntfy_api_key TEXT,
		theme TEXT CHECK (theme IS NULL OR theme IN ('rose-pine', 'rose-pine-dark', 'nord', 'tokyo-night', 'forest-night', 'matte-black'))
	) STRICT`); err != nil {
		return fmt.Errorf("create rebuilt settings table: %w", err)
	}
	...
	for _, name := range copiedColumns {
		if name == "theme" {
			selectColumns = append(selectColumns, `CASE WHEN theme = 'omarchy' THEN 'matte-black' WHEN theme = 'og' THEN 'rose-pine-dark' ELSE theme END AS theme`)
		} else {
			selectColumns = append(selectColumns, name)
		}
	}
	statements := []string{
		`INSERT INTO settings_theme_19 (` + strings.Join(copiedColumns, ", ") + `)
