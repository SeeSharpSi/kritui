...
- `db.InsertChat` and `db.InsertMessage` persist chats and `llm.Message` values, including optional usage, provider metadata, and per-message prompt append text. `db.GetMessageSnapshot` captures ordered history plus a storage digest; `db.AppendCompletion` obtains a writer lock, rejects changed history, chooses the next stored position, and atomically appends generated messages. `db.AllocateChat` creates a chat preloaded with default-enabled tools and no appends; prompt append definitions live in settings with built-in text embedded from `db/default_appends/*.md`, while selected IDs are stored per chat and expanded only when completion history is sent to the model. `db.GetDefaultEnabledTools`/`db.SetDefaultEnabledTools` back the settings UI, which also stores default model, max tool rounds, and prompt appends. Existing DBs get versioned migrations in `migrateDatabase` on startup.
