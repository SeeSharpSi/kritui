...
- `static/` is embedded into the executable. Restart `go run .` after asset changes. htmx and `htmx-ext-sse` v2.2.4 are vendored there; `static/app.js` contains persistent presentation behavior and completion network-error recovery. There is no npm build pipeline.
...
- Conversations validate complete assistant responses before mutating history or executing tools. Valid function calls are executed through the registry, appended as `tool` messages with matching `tool_call_id` values, and sent back to the model. Consecutive tool-call chains are limited to 16 executed rounds.
