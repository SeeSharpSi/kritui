...
- `server.go` is the HTTP entrypoint: `GET /` renders the page, `GET /healthz` checks local database readiness, history and chat mutation routes return fragments, `POST /messages` dispatches leading slash commands or accepts a user message, `POST /messages/complete` returns the final message, `POST /messages/retry` prepares a failed completion retry, `GET /messages/tools` streams tool-call HTML over SSE, `GET /sw.js` serves the service worker for root scope, and `GET /static/` serves embedded assets.
